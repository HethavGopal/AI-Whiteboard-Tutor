...
**Derivative / limits / continuity lesson layout convention (wipe-and-draw):** the prompt splits graph-based calculus lessons into two phases. Phase 1 (steps 1-4) stacks symbolic narration text on the left column at fixed y-coordinates (80, 115, 160, 205+30*i, 295). Phase 2 (step 5) begins with an `erase` action targeting every Phase-1 label, then draws axes + curve + (tangents | discontinuity markers) on the full canvas. This prevents narration text and graph shapes from ever occupying the same space. The prompt also enforces a topic-agnostic safety rule — **AXES IMPLIES PRIOR-TEXT ERASE**: any step whose `drawActions` contain an `axes` action MUST start with an `erase` of every text-shape `semanticLabel` from earlier steps. A separate "purely abstract" clause covers questions with no possible visual (e.g. "explain the epsilon-delta definition", "name the rules of differentiation"); those produce a text-only lesson with no `axes`/`plot_function`/`tangent_line`/`point` actions. Conceptual but **visualizable** questions (e.g. "explain how to find if a limit exists", "what is a derivative geometrically") must NOT route to the purely-abstract clause — they auto-pick the simplest illustrative example function and run the full two-phase recipe. If you add more graph-based calculus topics, follow the same two-phase erase pattern and the recipe block lives in [lib/k2-lesson-service.ts](lib/k2-lesson-service.ts) (canonical) — mirror it into [lib/gemini-lesson-service.ts](lib/gemini-lesson-service.ts) since both prompts must stay in sync.
